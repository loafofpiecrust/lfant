...
Properties::Properties(ifstream &stream, string type, string id, Properties *parent) :
...
void Properties::LoadStream(ifstream& stream)
{
	...
	while(stream.good())
	{
		...
		if(pos != string::npos)
		{
			...
			to_lower(name);
			Log(name, "=", value);
			Set(TrimSpace(name), TrimSpace(value, true));
			name = "";
			value = "";
			continue;
		}
		...
		Log("Splitting line at '{}' and spaces.");
		// Namespacing
		...
		{
			Log("No starting brace on this line.");
			value = toks[1];
		}
		...
		{
			to_lower(name);
			Log("Next line '{', Begin namespace '"+value+"' of type '"+name+"'.");
			children.push_back(ptr<Properties>(new Properties(stream, name, value, this)));
		}
		else
		{
...
void Properties::SaveStream(ofstream &stream)
{
...
void Properties::SkipSpace(ifstream &stream)
{
