...
bool SQVM::DerefInc( SQInteger op, SQObjectPtr& target, SQObjectPtr& self, SQObjectPtr& key, SQObjectPtr& incr, bool postfix, SQInteger selfidx )
{
	...
	if( !Get( tself, tkey, tmp, false, selfidx ) )
	{
		return false;
	}
		...
		for( ;; )
		{
			...
			switch( _i_.op )
			{
				...
				case _OP_PREPCALLK:
					...
					continue;
				case _OP_GETK:
					if( !Get( STK( arg2 ), ci->_literals[arg1], temp_reg, false, arg2 ) )
					{
						SQ_THROW();
					}
					_Swap( TARGET, temp_reg );  //TARGET = temp_reg;
...
SQInteger SQVM::FallBackGet( const SQObjectPtr& self, const SQObjectPtr& key, SQObjectPtr& dest )
{
	switch( type( self ) )
	{
		...
		case OT_USERDATA:
			...
			{
				if( Get( SQObjectPtr( _delegable( self )->_delegate ), key, dest, false, DONT_FALL_BACK ) )
				{
					return FALLBACK_OK;
				}
			}
