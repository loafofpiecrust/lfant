...
float maxblur = 0.25; //clamp value of max blur (0.0 = no blur,1.0 default)
...
vec3 color(sampler2D tex, vec2 coords,float blur) //processing the sample
{
	vec3 col = vec3(0.0);
...
vec3 depthOfField(sampler2D tex, vec2 pTextureSize, vec2 pCamRange, vec2 pUV, float focalDepth, float focalLength, float fstop, float focus)
{
	...
	else
	{
		...
		for (int i = 1; i <= rings; i += 1)
		{
			...
			for (int j = 0 ; j < ringsamples ; j += 1)
			{
				...
				col += color(tex, pUV + vec2(pw*w,ph*h),blur)*mix(1.0,(float(i))/(float(rings)),bias)*p;
